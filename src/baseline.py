...
def load_splits():
    ...
    log.info(
        f"Données chargées — train:{len(train):,} val:{len(val):,} test:{len(test):,}"
    )
...
def plot_confusion_matrix(y_true, y_pred, class_names: list, split: str):
    ...
    ax.set_title(
        f"Matrice de confusion normalisée — Baseline TF-IDF ({split})",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_per_class_f1(report_dict: dict, class_names: list, split: str):
    f1_scores = {
        cls: report_dict[cls]["f1-score"] for cls in class_names if cls in report_dict
    }
    ...
    ax.axvline(
        ...
        lw=1.5,
        linestyle="--",
        label=f"Macro avg: {np.mean(list(f1_scores.values())):.3f}",
    )
    ...
    ax.set_title(
        f"F1-score par classe — Baseline TF-IDF ({split})",
        fontsize=13,
        fontweight="bold",
    )
    ax.legend()
...
def main(args):
    ...
    with mlflow.start_run(run_name="tfidf-logreg-baseline"):
        ...
        mlflow.sklearn.log_model(
            pipeline, "model", registered_model_name="news-classifier-baseline"
        )
