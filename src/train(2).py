...
def plot_training_curves(history: dict):
    ...
    axes[0].plot(epochs, history["train_loss"], "o-", color="#5B4FCF",
                 lw=2, label="Train loss")
    axes[0].plot(epochs, history["val_loss"],   "o-", color="#D85A30",
                 lw=2, label="Val loss")
    ...
    axes[1].plot(epochs, history["val_f1"],  "o-", color="#1D9E75",
                 lw=2, label="Val F1 macro")
    axes[1].plot(epochs, history["val_acc"], "o-", color="#5B4FCF",
                 lw=2, linestyle="--", label="Val Accuracy")
    axes[1].axhline(0.88, color="#D85A30", lw=1.5, linestyle=":",
                    label="Objectif F1=0.88")
    ...
    plt.suptitle("DistilBERT Fine-tuning — Courbes d'entraînement",
...
def main(args):
    ...
    # ── Run MLflow ────────────────────────────────────────────
    with mlflow.start_run(run_name=f"distilbert-e{args.epochs}-lr{args.lr}-bs{args.batch_size}"):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_f1 > best_val_f1:
                ...
                log.info(f"  ✅ Nouveau meilleur modèle sauvegardé (F1={best_val_f1:.4f})")
