import os, json, time, logging, argparse, warnings
...
MODELS_DIR = Path("models/distilbert"); MODELS_DIR.mkdir(parents=True, exist_ok=True)
CHECKPOINT_DIR = MODELS_DIR / "checkpoints"; CHECKPOINT_DIR.mkdir(exist_ok=True)
REPORTS_DIR = Path("reports/figures"); REPORTS_DIR.mkdir(parents=True, exist_ok=True)
DEVICE = torch.device("cpu")

class NewsDataset(Dataset):
    def __init__(self, df, tokenizer, max_length=128):
        log.info(f"  Pré-tokenisation de {len(df):,} exemples...")
        enc = tokenizer(df["text"].tolist(), max_length=max_length, padding="max_length", truncation=True, return_tensors="pt")
...
def train_epoch(model, loader, optimizer, scheduler):
    model.train(); total_loss = 0.0; n = len(loader); t0 = time.time()
    for i, batch in enumerate(loader):
        ...
        if (i+1) % 100 == 0:
            ...
            log.info(f"  Step {i+1}/{n} — loss: {out.loss.item():.4f} — ETA: {eta:.1f}min")
    return total_loss / n

@torch.no_grad()
def evaluate(model, loader):
    model.eval(); preds, labs, total_loss = [], [], 0.0
    for batch in loader:
        out = model(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"], labels=batch["label"])
...
def main(args):
    ...
    log.info(f"Train:{len(train_df):,} Val:{len(val_df):,} Test:{len(test_df):,} Classes:{num_labels}")
    ...
    model = DistilBertForSequenceClassification.from_pretrained(
        "distilbert-base-uncased", num_labels=num_labels, id2label=id2label,
        label2id={v:k for k,v in id2label.items()}, dropout=0.2, seq_classif_dropout=0.2)

    optimizer = AdamW(model.parameters(), lr=args.lr, weight_decay=0.05)
    total_steps = len(train_loader) * args.epochs
    warmup_steps = int(0.06 * total_steps)
    scheduler = get_linear_schedule_with_warmup(optimizer, warmup_steps, total_steps)
    log.info(f"Steps/epoch:{len(train_loader)} Total:{total_steps} Warmup:{warmup_steps}")
    ...
    with mlflow.start_run(run_name=f"distilbert-cpu-fast{args.fast}-e{args.epochs}"):
        mlflow.log_params({"fast_mode": args.fast, "train_size": len(train_df),
                           "lr": args.lr, "batch_size": args.batch_size, "epochs_max": args.epochs})

        history = {"train_loss":[], "val_loss":[], "val_f1":[], "val_acc":[]}
        best_val_f1 = 0.0; no_improve = 0; best_path = MODELS_DIR / "best_model"
        t0 = time.time()
        ...
        for epoch in range(1, args.epochs+1):
            ...
            log.info(f"  Train loss:{train_loss:.4f} | Val loss:{val_loss:.4f} | F1:{val_f1:.4f} | Acc:{val_acc:.4f}")
            mlflow.log_metrics({"train_loss":round(train_loss,4),"val_loss":round(val_loss,4),"val_f1":round(val_f1,4)}, step=epoch)
            ...
            if val_f1 > best_val_f1:
                ...
                if no_improve >= args.patience:
                    log.warning(f"  🛑 Early stopping epoch {epoch}"); break
        ...
        print("="*60)
        mlflow.log_metrics({"test_f1_macro":round(test_f1,4),"test_accuracy":round(test_acc,4),"training_minutes":round(total_min,1)})
        metrics = {"model":"distilbert-base-uncased","fast_mode":args.fast,"epochs_run":len(history["train_loss"]),
                   "test_f1_macro":round(test_f1,4),"test_accuracy":round(test_acc,4),
                   "best_val_f1":round(best_val_f1,4),"baseline_f1":0.6515,
                   "delta_f1":round(test_f1-0.6515,4),"history":history,"class_names":class_names}
        with open(MODELS_DIR/"training_metrics.json","w") as f: json.dump(metrics,f,indent=2)
        log.info(f"  Métriques → models/distilbert/training_metrics.json")
